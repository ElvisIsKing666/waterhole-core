<turbo-frame id="post-feed" class="post-feed stack gap-lg" target="_top" data-controller="post-feed"
    ...
    <div>
        <form class="post-feed__refresh animate-appear" data-post-feed-target="newActivity" data-turbo-frame="post-feed"
            hidden>
            <div>
                <button type="submit" class="btn btn--sm bg-activity" data-action="post-feed#scrollToTop">
                    @icon('tabler-refresh')
            ...
            <div class="post-feed__content {{ $feed->layout->wrapperClass() }}">
                <x-waterhole::infinite-scroll :paginator="$posts">
                    @foreach ($posts as $post)
                        ...
                        <x-dynamic-component :component="$feed->layout->itemComponent()" :post="$post" />
                    @endforeach
