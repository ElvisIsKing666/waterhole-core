...
<body {{ $attributes->merge(['data-route' => request()->route()->getName(),]) }}>
    ...
    --}}
    <ui-modal id="modal-element" class="modal" hidden data-controller="modal" data-action="
                turbo:before-stream-render@document->modal#hide
                turbo:before-render@document->modal#hide" data-turbo-permanent>
        {{-- https://github.com/hotwired/turbo/pull/445#issuecomment-995305287 --}}
        <turbo-frame data-id="modal" data-controller="turbo-frame" class="modal__frame" data-modal-target="frame"
            data-action="
                    turbo:submit-start->modal#loading
                    turbo:before-fetch-request->modal#loading
                    turbo:frame-render->modal#loaded" aria-labelledby="dialog-title" disabled></turbo-frame>
